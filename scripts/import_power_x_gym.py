...
db_paths = [
    r"E:\gymflow\gym-management-system\backend\gymflow_dev.db",
    r"E:\gymflow\gym-management-system\gymflow_dev.db"
]
uploads_paths = [
    r"E:\gymflow\gym-management-system\backend\uploads",
    r"E:\gymflow\gym-management-system\uploads"
]
        ...
        if is_sl:
            if i + 1 < total_rows:
                ...
                if name_clean or phone_clean:
                    # Find image anchored in this block
...
def import_into_db(db_path, members):
    ...
    # 1. Retrieve or create gym
    cursor.execute("SELECT id, name FROM gyms")
    gyms = cursor.fetchall()
    
    if not gyms:
        gym_id = "ea668b94-2ee5-40ff-a9c3-1e15908e8c57"
        print(f"No gyms found. Creating default gym: Power X Gym (ID: {gym_id})")
        now_str = datetime.now(timezone.utc).isoformat()
        cursor.execute("""
            INSERT INTO gyms (id, name, slug, phone, email, is_active, created_at, updated_at)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)
        """, (gym_id, "Power X Gym", "power-x-gym", "9999999999", "contact@powerxgym.com", 1, now_str, now_str))
        
        # Associate existing users with this gym
        cursor.execute("UPDATE users SET gym_id = ?", (gym_id,))
        print("Associated all existing users with the new gym.")
    else:
        gym_id = gyms[0][0]
        gym_name = gyms[0][1]
        print(f"Found existing gym: {gym_name} (ID: {gym_id})")
        
    # 2. Get existing phone numbers in members table to avoid duplicate insert
    ...
    print(f"Found {len(existing_phones)} existing member phone numbers in DB.")
    ...
    conn.commit()
    conn.close()
    print(f"Database import complete: {imported_count} imported, {skipped_count} skipped, {photo_count} photos linked.")

def main():
    print("Starting import script...")
    members = parse_excel_members()
    for db_path in db_paths:
        import_into_db(db_path, members)
